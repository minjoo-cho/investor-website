...
import { ArrowUpRight, DollarSign, ExternalLink } from 'lucide-react';
...
// 투자자 카드 컴포넌트
const InvestorCard = ({ investor, onViewDetails }) => {
  return (
    <div 
    ...
    >
      <div className="relative h-48">
        ...
        {/* 포트폴리오 링크 */}
        <div className="absolute top-2 right-2 bg-blue-100 text-blue-700 text-xs px-2 py-1 rounded-full flex items-center">
...
// 투자자 상세 페이지
const InvestorDetailPage = ({ investor, onBack }) => {
  return (
    <div className="max-w-7xl mx-auto px-4 py-4">
      <button 
        onClick={onBack}
      ...
      <div className="bg-white rounded-lg shadow-md overflow-hidden mb-6">
        <div className="h-56 relative">
          ...
          <div className="absolute top-4 right-4">
            <a 
              href="#" 
              className="bg-blue-600 text-white px-4 py-2 rounded-full text-sm flex items-center gap-1 hover:shadow-lg transition-shadow"
            >
...
const App = () => {
  ...
  const handleViewDetails = (investorId) => {
    setSelectedInvestor(investors.find(inv => inv.id === investorId));
  };
